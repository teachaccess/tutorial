...
<Editor
  checkExercise={(exercise) => {
    const tableElement = exercise.querySelector("table")
    if (!tableElement) {
      return "Use a semantic table element."
    }
    const trElements = exercise.querySelectorAll("tr")
    if (!trElements.length) {
      return "Are you using semantic tr tags?"
    }
    const thElements = exercise.querySelectorAll("th")
    if (!thElements.length) {
      return "Are you using semantic th tags?"
    }
    const tdElements = exercise.querySelectorAll("td")
